...
import colorPicker.math.model.HSV
...
data class HsvColor(
    ...
    fun getComplementaryColor(): List<HsvColor> {
        return listOf(
            ...
            this.copy(
                hue = (hue + 180) % 360,
                saturation = (saturation + 0.2f).coerceAtMost(1f),
                value = (value - 0.3f).coerceIn(0.0f, 1f)
            )
        )
    }
    ...
    fun getSplitComplementaryColors(): List<HsvColor> {
        return listOf(
            this.copy(
                hue = (hue + 150) % 360,
                saturation = (saturation - 0.05f).coerceIn(0.0f, 1f),
                value = (value - 0.3f).coerceIn(0.0f, 1f)
            ),
            this.copy(
                hue = (hue + 210) % 360,
